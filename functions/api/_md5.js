/* MD5 — нужен для подписи pg_sig во Freedom Pay.
   В Workers нет Web Crypto MD5, поэтому реализация своя. */

function toWords(s) {
  const bytes = new TextEncoder().encode(s);
  const n = ((bytes.length + 8) >> 6 << 4) + 16;
  const w = new Array(n).fill(0);
  for (let i = 0; i < bytes.length; i++) w[i >> 2] |= bytes[i] << ((i % 4) << 3);
  w[bytes.length >> 2] |= 0x80 << ((bytes.length % 4) << 3);
